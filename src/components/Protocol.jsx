...
import { FaFlask, FaDna } from 'react-icons/fa'; // Import icons from react-icons
    ...
    <div className="flex min-h-screen bg-gray-100">
      ...
      <div className="w-1/4 bg-gray-200 p-4">
        ...
        <ul className="space-y-2">
          {protocols.map((protocol, index) => (
            <li key={index}>
              <button
                className={`flex items-center p-2 text-gray-700 hover:bg-gray-300 rounded-lg text-left w-full ${
                  activeIndex === index ? 'bg-[#fb923c] text-white' : ''
                }`}
      ...
      <div className="w-3/4 p-4">
        {protocols.map((protocol, index) => (
          ...
          >
            <div className="accordion-content mb-4">
              {activeIndex === index && protocol.content}
            </div>
          </div>
        ))}
      </div>
